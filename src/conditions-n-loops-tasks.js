...
function canQueenCaptureKing(queen, king) {
  const lineX = Math.abs(king.x - queen.x);
  const lineY = Math.abs(king.y - queen.y);

  if (lineY === 0 || lineX === 0 || lineX === lineY) {
    return true;
  }
...
function convertToRomanNumerals(num) {
  const numbers = [1, 4, 5, 6, 9, 10];
  const romNum = ['I', 'IV', 'V', 'VI', 'IX', 'X'];
  let roman = '';
  ...
  if (num < 40) {
    ...
    for (let i = 0; i < tensPart; i += 1) {
      roman += 'X';
    }
    roman += convertToRomanNumerals(remainder);
    return roman;
  }
  return roman;
}
...
function rotateMatrix(matrix) {
  ...
  while (left < right) {
    for (let i = 0; i < right - left; i += 1) {
      const top = left;
      const bottom = right;
      const topLeft = matrixCopy[top][left + i];
